...
const modalText = `
        {/* Detail Modal */}
        {selectedRoom && (
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-200" onClick={() => setSelectedRoom(null)}>
            <div className="bg-white p-5 sm:p-8 rounded-3xl sm:rounded-[2rem] w-full max-w-sm max-h-[90%] sm:max-h-[90%] shadow-2xl border-4 border-indigo-50 flex flex-col" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex-1 min-h-0 overflow-y-auto space-y-2 sm:space-y-3 px-1 -mx-1">
              ...
              <div className="pt-5 sm:pt-6 shrink-0 mt-auto">
                <button 
                  className="w-full py-3 sm:py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-black transition-all shadow-xl shadow-indigo-100 active:scale-95" 
...
// Insert the modalText before the map container's closing div.
// The map container closing div is just before {isAdmin && (
text = text.replace("      </div>\n      \n      {isAdmin && (", modalText + "      </div>\n      \n      {isAdmin && (");

fs.writeFileSync('src/components/ResortMap.tsx', text);
console.log("Modal added back!");
